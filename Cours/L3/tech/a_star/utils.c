...
grid allocGrid(int x, int y){
  ...
  for(int i=0; i<x; i++){
    ...
    G.mark[i] = calloc(y, sizeof(*(G.mark[i]))); // initialise à 0
  }
...
void addRandomBlob(grid G, int type, int nb){
	...
	for (int i = 0 ; i < nb ; i++)
		G.value[random() % G.X][random() % G.Y] = type;
...
// Positionne G.start et G.end de façon aléaoire uniforme parmi toutes
// les valeurs de la grille de type "ts" (pour start) et de type "te"
// (pour end).
void randomPosition(grid *G, int ts, int te){
  int i,j,c;
  int ns,ne; // ns,ne=nombre de type ts,te
  int rs,re; // nombre aléatoire dans [0,ns[, [0,ne[ 

  ns=ne=0;
  rs=re=-1;

  for(c=0; c<2; c++){ // deux fois la même chose
    
    for(i=0; i<G->X; i++)
      for(j=0; j<G->Y; j++){
	if(G->value[i][j]==ts){
	  if(ns==rs) G->start.x=i,G->start.y=j,rs=-1;
	  ns++;
	}
	if(G->value[i][j]==te){
	  if(ne==re) G->end.x=i,G->end.y=j,re=-1;
	  ne++;
	}
      }
    
    // fin des 2 boucles i,j: on tire au hasard rs et re puis on
    // recommence le comptage et on s'arrête
    if(c==0) rs=random()%ns, re=random()%ne, ns=ne=0;
  }
}

void drawGrid(grid G){
  
  // saute le dessin si le précédent a été fait il y a moins de 20ms = 1/50s
